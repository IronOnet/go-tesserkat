...
type unmarshalTest struct{
	input string 
	want interface{} 
	wantErr error // if this is set, decoding must fail on any platform
	wantErr32bit error // if set, decoding must fail on 32 bit platforms (used for Uint tests)
}
...
func TestDecodeUint64(t *testing.T){
	for _, test := range decodeUint64Tests{
		...
		if dec != test.want.(uint64){
			t.Errorf("input %x: value mismatch: got %x, want %x", test.input, dec, test.want)
			continue
		}
	}
}
